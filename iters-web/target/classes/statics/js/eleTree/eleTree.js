...
layui.define([]/*"layer"*/, function (exports) {
    ...
    function Class(option) {
        ...
        this.elem="";
        this.data=[];
        this.relationCheckBox=this.option.relationCheckBox;//多选框选中是否关联选中
        this.showCheckbox=this.option.showCheckbox;//是否显示多选框
        this.drag=this.option.drag;//是否可拖拽
        this.accordion=this.option.accordion;//是否支持手风琴动画
        this.lazy=this.option.lazy;//是否懒加载
        this.loadData=this.option.loadData;//拦加载函数
        this.contextmenuList=[];    //右键菜单功能列表
    ...
    Class.prototype={
                    ...
                    ,'<div class="eleTree-node-content" style="padding-left: '+18*count+'px;">'
                        ...
                        ,(function() {
                            ...
                            if(self.showCheckbox){
                                s+='<input '+(val.id?'eleTree-id='+val.id:'')+' type="checkbox" name="eleTree-node"';
                                if(val.checked){
        ...
        eleTreeEvent: function() {
            ...
            $(this.elem).on("click",".eleTree-node-content",function(e) {
                ...
                var index=$(self.elem).find(".eleTree-node-content").index(this);
                $(self.elem).find(".eleTree-node-content").removeClass("eleTree-node-content-active");
                    ...
                    if(el.hasClass("layui-icon-triangle-d")){
                        ...
                        if(self.lazy && el.hasClass("lazy-icon")){
                            ...
                            self.loadData(d,function(childrenData) {
                                ...
                                $(self.elem).find(".eleTree-node-content").eq(index).addClass("eleTree-node-content-active");
                            })
                        }
